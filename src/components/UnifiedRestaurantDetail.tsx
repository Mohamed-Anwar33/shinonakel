...
const UnifiedRestaurantDetail = ({
  ...
  const handleDirections = () => {
    if (restaurant.mapsUrl) {
      ...
      openExternal(`https://www.google.com/maps/dir/?api=1&destination=${restaurant.latitude},${restaurant.longitude}`);
    } else if (restaurant.address) {
      openExternal(`https://www.google.com/maps/dir/?api=1&destination=${encodeURIComponent(restaurant.address)}`);
    }
  };
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <button
                      ...
                      className={`w-12 h-12 rounded-xl flex items-center justify-center transition-colors ${
                        canCall ? "bg-primary/10 hover:bg-primary/20" : "bg-muted/50 opacity-50 cursor-not-allowed"
                      }`}
                    >
                    ...
                    <button
                      ...
                      className={`w-12 h-12 rounded-xl flex items-center justify-center transition-colors ${
                        canShowDirections ? "bg-primary/10 hover:bg-primary/20" : "bg-muted/50 opacity-50 cursor-not-allowed"
                      }`}
                    >
                    ...
                    {deliveryApps.map((app) => (
                      <button
                        ...
                        className={`px-4 py-2 rounded-xl text-sm font-semibold border-2 transition-all ${
                          app.url ? "hover:scale-105 cursor-pointer bg-card" : "opacity-50 cursor-not-allowed bg-muted/50"
                        }`}
                        ...
                        <div className="flex justify-center gap-2">
                          {[1, 2, 3, 4, 5].map((star) => (
                            ...
                            >
                              <Star
                                className={`w-8 h-8 ${
                                  star <= userRating ? "fill-amber-400 text-amber-400" : "text-muted-foreground"
                                }`}
                              />
                            </button>
                          ))}
                      ...
                      <div className="flex items-center gap-0.5 mt-1 justify-center">
                        {[1, 2, 3, 4, 5].map((star) => (
                          <Star
                            key={star}
                            className={`w-4 h-4 ${
                              star <= Math.round(Number(avgRating)) ? "fill-amber-400 text-amber-400" : "text-muted"
                            }`}
                          />
                        ))}
                      </div>
