...
const Results = () => {
  ...
  useEffect(() => {
    ...
    const geocodeSequentially = async () => {
      ...
      for (const restaurant of restaurantsNeedingGeocode.slice(0, 5)) { // Limit to 5 at a time
  ...
  const transformedRestaurants = useMemo(() => {
    return restaurants.map((r, index) => {
      ...
      const avgRating = ratings.length > 0
        ? Number((ratings.reduce((sum, rating) => sum + rating.rating, 0) / ratings.length).toFixed(1))
        : 0;
      ...
      return {
        ...
        position: { top: `${15 + (index * 20) % 70}%`, right: `${20 + (index * 25) % 60}%` },
  ...
  const filteredRestaurants = useMemo(() => {
    let filtered = category === "الكل"
      ...
      : transformedRestaurants.filter(r => {
      ...
      <main className="max-w-md mx-auto px-4 pt-6">
        ...
        <div className="text-center mb-6 pt-4">
          {emojiParam ? (
            ...
            <span className="text-6xl">{getCuisineDisplay(category).emoji}</span>
          ) : (
                    ...
                    <div className="relative rounded-3xl overflow-hidden bg-gradient-to-b from-primary/20 to-primary/5 p-4">
                      ...
                      <div className="relative w-full aspect-[4/3] rounded-2xl overflow-hidden mb-4">
                        <img
                          ...
                          alt={language === "en" && featuredRestaurant.name_en ? featuredRestaurant.name_en : featuredRestaurant.name}
                        ...
                        >
                          <Heart
                            className={`w-5 h-5 transition-colors ${savedRestaurantIds.includes(featuredRestaurant.name) ? "fill-primary text-primary" : "text-muted-foreground"
                              }`}
                          />
                        </button>
                      ...
                      {featuredRestaurant.deliveryApps.length > 0 && (
                        <div className="bg-card rounded-2xl p-4 shadow-soft">
                          ...
                          <div className="flex items-center justify-center gap-2 flex-wrap">
                            {featuredRestaurant.deliveryApps.map((app) => (
                              <button
                                key={app.name}
                                onClick={() => {
                                  if (app.url) {
                      ...
                      {moreRestaurants.map((restaurant, index) => (
                        ...
                        >
                          <CompactRestaurantCard
                            name={language === "en" && restaurant.name_en ? restaurant.name_en : restaurant.name}
                            cuisine={`${getCuisineDisplay(restaurant.cuisine).emoji} ${getCuisineDisplay(restaurant.cuisine).name}`}
